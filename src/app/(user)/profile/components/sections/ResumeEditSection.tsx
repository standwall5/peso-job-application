...
export const ResumeEditSection: React.FC<ResumeEditSectionProps> = ({
  ...
  const handleParsedData = (data: ParsedResumeData) => {
    ...
    if (data.email && user?.email !== data.email) {
      // Note: Email is typically not editable due to verification
      console.log("Email from resume:", data.email);
    }
    if (data.phone) {
      // Phone would be updated in profile header section
      console.log("Phone from resume:", data.phone);
    }

    // Education
    if (data.education) {
      if (data.education.school) setEditSchool(data.education.school);
      if (data.education.degree) setEditDegree(data.education.degree);
      if (data.education.attainment)
        setEditEducationAttainment(data.education.attainment);
      if (data.education.location)
        setEditEducationLocation(data.education.location);
      if (data.education.start_date)
        setEditEducationStartDate(data.education.start_date);
      if (data.education.end_date)
        setEditEducationEndDate(data.education.end_date);
    }
    ...
    if (data.profile_introduction) {
      setEditIntroduction(data.profile_introduction);
    }
  };
  ...
  return (
    <>
      <form className={styles.editResume} onSubmit={onSave}>
        ...
        {/*<div className={styles.resumeUploadBanner}>
          <div className={styles.bannerContent}>
            <div className={styles.bannerIcon}>📄</div>
            <div className={styles.bannerText}>
              <h3>Save Time with Auto-Fill</h3>
              <p>
                Upload your resume to automatically fill in your information
              </p>
            </div>
          </div>
          <Button
