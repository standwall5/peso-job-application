...
export function useReferralLetter(
  ...
  const [bodyHTML, setBodyHTML] = useState(
    `<p><strong>Dear ${
      recipientName.split(" ").slice(1).join(" ") || "Sir/Madam"
    }:</strong></p>
      ...
      <p>In line with this, we respectfully refer to your office <strong>${
        jobseeker.applicant.sex === "Male" ? "Mr." : "Ms."
      } ${jobseeker.applicant.name}</strong> for the position of <strong>${
      application.job?.title || ""
    }</strong> or any job post commensurate to ${
      jobseeker.applicant.sex === "Male" ? "his" : "her"
    } qualification.</p>
  ...
  const handleDownloadPDF = async () => {
    ...
    if (resumeRef.current && jobseeker) {
      html2pdf()
        .set({
          margin: 0.5,
          filename: `Referral_Letter_${jobseeker.applicant.name.replace(
            /\s+/g,
            "_"
          )}.pdf`,
